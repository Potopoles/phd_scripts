...
class field:
    ...
    def _copy(self, ncd):
        ...
        copied = field(self.name, self.ncFile)
    ...
    def prepareForPlotting(self):
        hydrometeors = ['QC','QI','QV','QR','QG','QS']
        TTendencies = ['ATT_MIC', 'ATT_RAD', 'ATT_ADV', 'ATT_ZADV', 'ATT_TURB', 'ATT_TOT']
        QVTendencies = ['AQVT_MIC', 'AQVT_ADV', 'AQVT_ZADV', 'AQVT_TURB', 'AQVT_TOT']
    ...
    def loadAsDiurnal(self, agg_operation):
        ...
        hasTime = 0
        for key,dim in self.dims.items():
            if dim.function == 'time' and dim.size > 1:
                hasTime = 1
                tSplt = int(dim.size/2)				
                tInds = (dim.inds[:tSplt],dim.inds[tSplt:])
        ...
        for hr in range(0,24):
            #hr = 22
            insertInds[timeDimInd] = range(hr,hr+1)
            hrInds = []
            for ind,dt in enumerate(self.dims['time'].vals):
                if dt.hour == hr and dt.day != 20: # ATTENTION LIKE THIS ONLY VALID FOR MASTERTHESIS CASE (TODO)
                    hrInds.append(ind) 
            ...
            if agg_operation == 'MEAN':
                vals = np.nanmean(vals, axis=timeDimInd, keepdims=1)
            elif agg_operation == 'SUM':
                vals = np.nansum(vals, axis=timeDimInd, keepdims=1)

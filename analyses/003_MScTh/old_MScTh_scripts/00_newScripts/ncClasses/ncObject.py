...
class ncObject:
    ...
    def cutDimsToSubSpace(self):
        ...
        ## Refresh field if it is already selected
        #if self.field != None:
        #    self.selField(self.field.name)
    ...
    #def selField(self, fldName):
    #    """selects a variable of the ncFile and stores dimensions.
    #        Does not yet load values. This is done in loadField()"""
    #    self.field = fld.field(fldName, self.ncFile, self.dims)
    ...
    def saveToNC(self, outFilePath):
        """Saves copy of itself into a NC file and thus overwrites existing files!""" 
        from netCDF4 import Dataset
    ...
    def _copy(self):
        ...
        copied = ncObject(self.inpFilePath, self.name, self.res)
        copied.res = self.res 
        #copied.ncd = self.ncd._copy()
        for key,dim in self.dims.items():

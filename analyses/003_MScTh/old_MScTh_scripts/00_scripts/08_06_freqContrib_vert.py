...
for i in range(0,len(altInds)):
    ...
    for res in an.resolutions: 
        for mode in an.modes:
            ...
            freq[freq == 0] = np.nan
            out[res][mode][i,:] = freq*binsCentred/np.nansum(freq)
            out[res]['mask'][i,:][nPoint[0] < 50] = 0
...
lev_exp = np.arange(-5,0.01,0.2)
levels = np.power(10, lev_exp)
lev_exp = np.arange(-8,-1.01,0.2)
levels = np.asarray(np.power(10, lev_exp))
levels = np.append(-levels[-1::-1],levels)
    ...
    for resInd,res in enumerate(an.resolutions):
        ...
        if mode in ['', 'f']:
            CF = ax.contourf(binsCentred, altitudes, out[res][mode],
                            levels=levels, norm=colors.SymLogNorm(linthresh=1E-8), cmap='jet')
        else:

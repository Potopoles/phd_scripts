def plotVertProf(var_vP, varNCO, ax, meta):
    diurn = varNCO.dims['diurnal'].vals
    diurn = np.expand_dims(diurn, 0)
    diurn = np.repeat(diurn,len(subSpaceInds['altitude']),axis=0)

    PLOT = ax.plot(np.transpose(diurn), var_vP)
    ax.axvline(x=0, color=(0.5,0.5,0.5), linestyle='-', linewidth=1)
    altInds = subSpaceInds['altitude']
    altitude = []
    for altInd in altInds:
        if altInd > 60:
            altitude.append(6000 + (altInd-60)*1000)
        else:
            altitude.append(altInd*100)
    ax.legend(PLOT, altitude)
    ax.set_ylim([meta['min'], meta['max']])
    ax.grid()
...
for res in ress:
    ...
    for mode in modes:
        ...
        subSpaceInds['altitude'] = [5, 10, 15, 20, 30, 40]

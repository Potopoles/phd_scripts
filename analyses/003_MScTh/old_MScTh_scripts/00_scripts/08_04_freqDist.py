...
if varName == 'zQS':
    ...
    altInds = [10,20,30,40,50,60,61,62]
elif varName == 'zQV':
...
for startHght in altInds:
    ...
    plt.xlabel(varName[1:]+' [kg/kg]')
    plt.ylabel('relative frequency')
    plt.xlim((binMin,binMax))
    plt.ylim((1E-8,1E0))
    plt.title(varName[1:]+' Frequency Distribution at Altitude '+str(altitude))
    plt.grid()
    if i_plot == 2:
        outName = varName[1:]+'_dist_alt_'+str(altitude)+'.png'
    ...
    for i in range(0,len(an.resolutions)):
        ...
        if varName == 'zQS':
            ...
            ax.set_ylim((-2,2))
        elif varName in ['zQC', 'zQG']:
            ax.set_ylim((-1,1))

        ax.set_xlim((binMin,binMax))
        ax.grid()
        ax.set_title(str(an.resolutions[i]))
    plt.suptitle(varName[1:]+' Frequency Distribution at Altitude '+str(altitude))
    plt.subplots_adjust(left=0.07, right=0.95, top=0.92, bottom=0.08, wspace=0.27, hspace=0.33)
    if i_plot == 2:
        outName = varName[1:]+'_dist_and_diff_alt_'+str(altitude)+'.png'
        plt.savefig(outPath + outName)
    elif i_plot == 1:
        plt.show()
    plt.close(fig)

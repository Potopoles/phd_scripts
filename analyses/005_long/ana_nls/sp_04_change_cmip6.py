...
#time_periods_cmip6 = [{'first_date':datetime(1985,1,1),
#                       'last_date':datetime(1985,1,31)},
#                       {'first_date':datetime(2070,1,1),
#                       'last_date':datetime(2070,1,1)}
#                       ]
...
name_dict[agg_level] = scenario

cfg = {
    'serial_time_plt_sels': serial_time_plt_sels,
    'sub_dir':              'sp_change_cmip6',
    'name_dict':            name_dict,
    'figsize':              (nlp['figsize']['{}x{}'.format(
                                    nrows,ncols).format(nrows,ncols)][0],
                             nlp['figsize']['{}x{}'.format(
    ...
    'panels':
    {
        '0,0':
        {
            'i_recompute':  i_recompute[cosmo_3],
            'time_periods': time_periods_cosmo,
            'mem_keys':     [{'diff':['COSMO_3.3_pgw', 'COSMO_3.3_ctrl']}],
        },
    }
}
    ...
    for col_ind in range(ncols):
        if (col_ind == 0) and (row_ind == 0):
            ...
            for model_key in  models_cmip6:
                diffs.append({'diff':['{}_{}'.format(model_key, scenario),
                                      '{}_historical'.format(model_key),]})
            ...
            if mod_ind < len(models_cmip6):
                ...
                cfg['panels']['{},{}'.format(row_ind, col_ind)] = {
                    'i_recompute':  i_recompute[model_key],
                    'mem_keys':     [{'diff':['{}_{}'.format(model_key, scenario),
                                             '{}_historical'.format(model_key),]}],
                }
                mod_ind += 1
